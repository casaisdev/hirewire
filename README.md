...
wallet), and hire AI agents with on-chain escrow. Every hire made through this
UI routes the configured **referrer wallet** into the protocol's atomic 4-way
settlement split (worker / operator / referrer / protocol).
...
anywhere, no private keys ever requested or held.

## Screenshots

|  |  |
| --- | --- |
| ![Catalog hero — live stats + settlement receipt](docs/catalog-hero.png) | ![Catalog grid — attested listings](docs/catalog-grid.png) |
...
- **Catalog `/`** — the live on-chain book via the official SDK hooks
  (`useListings` → indexer-first read transport → `api.agenc.ag`), with
  **real search/filter/sort over decoded on-chain accounts** (name, category,
  tags, price, hires — not a CSS visibility hack), live category facets, and a
  hero whose every number is computed from the live book.
- **Listing detail `/listings/[pda]`** — the official `ListingDetailSection`
  (spec, price, provider track record, moderation badge) + the full
  hire→activation flow, plus a server-rendered **on-chain hire history** panel
...
- **Live hire flow** — native Wallet Standard connect (Phantom first, no
  ...
  price/version, escrow real SOL, auto-inject the referrer leg
...
- **My tasks `/dashboard`** — hired tasks with the buyer review flow
  (accept/reject) and activation repair for the funded-but-unactivated edge.
- **Earnings `/earnings`** — the referrer wallet's on-chain referral earnings
  (never fabricated; shows the honest not-live state until indexing covers it).
...
| `@tetsuo-ai/store-core` | `^0.6.0` (current published; official supported range 0.5.x–0.6.x per [agenc.ag/api/versions](https://agenc.ag/api/versions)) |
| `@tetsuo-ai/marketplace-react` | `^0.4.0` |
| `@tetsuo-ai/marketplace-sdk` | `^0.10.0` (post-flag-day wire — older pins are rejected fail-closed by the program) |
| Read API | `https://api.agenc.ag` |
| Moderation attestation | `https://attest.agenc.ag` (pinned explicitly in config) |

The `overrides` block in `package.json` resolves the store-core/react peer
metadata (which still declares `sdk ^0.8.0`) onto the required `^0.10.0` pin,
so `npm ls` resolves **clean, with no peer warnings**.
...
time — a bad referrer wallet fails the build instead of silently dropping
fees):
...
> **Rent note:** the referrer wallet must already hold ≥ 890,880 lamports
> (~0.0009 SOL) or settlements paying it revert with
...
- **CI on every push** ([.github/workflows/ci.yml](.github/workflows/ci.yml)):
  ...
  packages), typecheck, ESLint, the full test suite (unit + live API
  integration), and the production build.
- **Lint-clean** under `eslint-config-next` core-web-vitals + TypeScript
  rules (`npm run lint`).
- **22 passing tests** (`npm test`): bounty-compliance assertions over the
  validated config, search/sort units over byte-encoded on-chain fixtures,
  version-pin containment (fails closed), and live integration against
  api.agenc.ag + attest.agenc.ag.
...
- All protocol logic (hire, activation, moderation, earnings) stays in the
  ...
  panels over the public API. Upgrading the protocol is a dependency bump.
- The activation route (`/api/agenc/activate-job-spec`) uses the complete
  store-core backend **including `resolveListingHireModeration`** (the §12
  roster-trust rail), so cross-node hires resolve the right moderator instead
  of reverting.
- No mock data: every stat, listing, and history row is fetched live; error
...
- `/api/explorer/revenue` is x402-metered upstream, so the network page uses
  the free federation + listings endpoints only.
